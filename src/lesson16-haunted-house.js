// lesson16-haunted-house.js
import * as THREE from 'three'
import { OrbitControls } from 'three/examples/jsm/controls/OrbitControls.js'
// import { Timer } from 'three/addons/misc/Timer.js' // Timer is now part of core. No need to import it specifically
...
export default function init(canvas){
    ...
    // Inform Three.js that those textures need to be repeated
    ...
    // Color textures (not data textures like the normal map, AO map, roughness map, etc.) are being encoded in sRGB to optimise how the
    // color is stored according to how color is perceived by human eyes
    ...
    const floor = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({
            ...
            displacementScale: 0.3, // Lower the effect
            displacementBias: -0.2 // Offset the displacement to the vertices
        })
    );
    // Add to gui to tweaks displacement texture bias and scale
    ...
    for(let i = 0; i < 30; i++)
    {
        ...
        // Add the grave mesh to the graves group
        graves.add(grave);
    }
    ...
    // Which light can cast shadows
    directionalLight.castShadow = true;
    ghost1.castShadow = true;
    ghost2.castShadow = true;
    ghost3.castShadow = true;

    // Which object can cast or receive shadows
    ...
    // To make it look right, we first need to add some parameters to this sky instance
    ...
    // The can't be seen because it's too small for now. Let's scale it as it inherits from mesh
    sky.scale.set(100, 100, 100)

    /**
     * Fog
     */
    // // Simple fog : useful if we want perfect control over where the fog starts and ends
    // scene.fog =  new THREE.Fog('#ff0000', 1, 13);

    // FogExp2
    scene.fog = new THREE.FogExp2('#0434f4', 0.1);
